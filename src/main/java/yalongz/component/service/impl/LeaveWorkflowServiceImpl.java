...
public class LeaveWorkflowServiceImpl extends BaseServiceImpl<Leave, LeaveWorkflowDao> implements LeaveWorkflowService {
	...
	public ProcessInstance startWorkflow(Leave entity, Map<String, Object> variables) {
		int id = this.baseDao.SaveLeave(entity);
//		leaveManager.saveLeave(entity);
	...
	public List<Leave> findTodoTasks(String userId, Page<Leave> page, int[] pageParams) {
		...
		for (Task task : tasks) {
			...
			Leave leave = this.getBaseDao().getLeave(new Long(businessKey));
